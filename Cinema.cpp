...
std::vector<Proiectie*> Cinema::filtrare_smart(const CriteriiCautare& c) const {
    ...
    for (const auto& p : toate) {
        ...
        if (match) {rezultat.push_back(p.get());}
    }
    return rezultat;
}
...
std::vector<Proiectie*> Cinema::filtreaza_pentru_copii() const {
    ...
    for (const auto& p_ptr : ref) {
        if (p_ptr->getFilm().esteAnimatie()) {
            proiectie_copii.push_back(p_ptr.get());
        }
    }
...
void Cinema::incarca_din_fisier(const std::string &nume_fisier) {
    ...
    while (fin >>std::ws >> tipEveniment >> titlu >> gen >> durata >> zi >> ora >> tip >> nr_sala >> capacitate >> este_animatie) {
        Film f(titlu, gen, durata, este_animatie);
        Sala s(nr_sala, capacitate);

        std::unique_ptr<Proiectie> p_nou;

        if (tipEveniment == 'P') {
            p_nou = std::make_unique<PremieraSpeciala>(f, s, zi, ora, tip);
        } else if (tipEveniment == 'M') {
            std::string extra1, extra2;
            fin >> extra1 >> extra2;
            std::vector<std::string> titluriExtra = {extra1, extra2};
            p_nou = std::make_unique<MaratonFilme>(f, s, zi, ora, tip, 3, titluriExtra);
        } else {
            p_nou = std::make_unique<ProiectieStandard>(f, s, zi, ora, tip);
        }

        proiectii.adauga(std::move(p_nou));
    }
    fin.close();
}

std::vector<Proiectie*> Cinema::get_program_sortat() const {
    std::vector<Proiectie*> program_sortat;
    const auto& ref = proiectii.getReferinta();

    for (const auto& ptr : ref) {
...
std::vector<Proiectie*> Cinema::filtreaza_pe_gen(const std::string &gen_cautat) const {
    ...
    const auto& ref = proiectii.getReferinta();
    for (const auto& p_ptr : ref) {
        if (p_ptr->getFilm().getGen() == gen_cautat) {
            lista_filtrata.push_back(p_ptr.get());
        }
    }
...
Proiectie* Cinema::get_proiectie(int index) {
    if (index <  0 || (size_t)index >= proiectii.getReferinta().size()) {
        return nullptr;
    }
    return proiectii.getReferinta()[index].get();
}
...
void Cinema::actualizare_sala_originala(const Proiectie &proiectie_modificata) {
    ...
    for (auto& p : toate) {
        if (p->getFilm().getTitlu() == proiectie_modificata.getFilm().getTitlu() &&
            p->getZi() == proiectie_modificata.getZi() &&
            p->getOra() == proiectie_modificata.getOra())
            {
                p->getSala() = proiectie_modificata.getSala();
                break;
            }
    }
}
